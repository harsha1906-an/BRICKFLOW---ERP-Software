  ...
  ({ entity, jsonData }) =>
    async (dispatch) => {
      let data = await request.patch({
        entity: `${entity}/profile/update`,
        jsonData
      });

      if (data.success === true) {
        // Get existing auth state to preserve the token
        const existingAuth = JSON.parse(window.localStorage.getItem('auth') || '{}');

        dispatch({
          type: actionTypes.REQUEST_SUCCESS,
          payload: {
            ...data.result,
            token: existingAuth.current?.token, // Preserve existing token
          },
        });
